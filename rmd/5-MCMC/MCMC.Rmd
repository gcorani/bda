...
<!-- for very strange reason, this is needed to compile the first math eq  -->
\sbox0{$x$}
...
f(\theta \mid D) =
\frac{
\underbrace{f(\theta)}_{\text{prior}} 
\underbrace{\mathcal{L}(D \mid \theta)}_{\text{likelihood}}
}
{
{\underbrace{f(D)}_{\text{marg lik}}}
}
\end{align*}
...
\begin{align*}
\theta_{\text{proposed}} & = \text{min} (\theta_{\text{proposed}}, 0.9999) \\
\theta_{\text{proposed}} & = \text{max} (\theta_{\text{proposed}}, 0.0001) 
\end{align*}
