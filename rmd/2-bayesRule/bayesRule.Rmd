...
* Examples of events and probabilities related to a fair dice: \bigskip
  ...
  * $E_1$: it lands 4 ($P$=1/6)
  * $E_2$: it lands odd: 1 or 3 or 5.  ($P$=1/2)
  * $E_3$: it lands 2 or 4.  ($P$=1/3)
...
* $A$ = it’s 0 degrees Celsius tonight. \bigskip
...
* $A$ = the first child is a boy \bigskip
...
* For instance:
  ...
  * Since the coin does not affect the performance of rider Q, the two probabilities are the same.
...
* Moreover, $$P(A= \text{fake}) = P(\text{fake},!) + P(\text{fake},~!)$$.  
...
* Unobserved variable $A$ \bigskip

* Observable variable $B$ \bigskip


Requires:

  * the prior  $P(A)$ \bigskip
  
  * the conditional distribution $P(B|A)$ \bigskip

* Given $B=b$, the posterior   of $A$ is: \bigskip

**RIVEDI**

\begin{align*}
 p(A|b) & = \frac{p(A)p(A \mid b)}{p(b)} = 
\frac{p(A)p(y \mid H)}{\sum_a p(a,b)} \\
& p(b)>0
\end{align*}
...
* $p(A \mid b)$ represents the distribution of the possible outcome $A$  given $B=b$.

* When we have a specific observation $b$, we get the function $p(a|b)$,  the likelihood. 
...
|  | test negative | test positive |
| :-: | :-: | :-: |
$P( T \mid \text{healthy})$ | infected | .975 | .025  |
...
\begin{align*}
P(\text{infected}|\text{positive}) & = \frac{P(\text{infected}) P(\text{positive} \mid {infected})}{P(\text{positive})} \\
