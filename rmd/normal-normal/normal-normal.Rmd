...
* The likelihood assumes independence of the observations $y_1, y_2, ..., y_n$
...
In the hyppocampus example, the normal-normal model is:
...
\bigskip 
* The only parameter of this model is $\mu$.
...
* Specify and plot the corresponding likelihood function of $\mu$.
  * Hint: define a range for the  values of $\mu$ and make it a grid of e.g. 100 points 
  * Compute the likelihood for each point of the grid.
...
$$
\mu|\vec{y} \; \sim \;  N\bigg(\mu'
\underbrace{\frac{\sigma^2}{n\sigma_{\mu}^2+\sigma^2}}_{w} + \bar{y}
\underbrace{\frac{n\sigma_{\mu}^2}{n\sigma_{\mu}^2+\sigma^2}}_{1-w}, \; \frac{\sigma_{\mu}^2\sigma^2}{n\sigma_{\mu}^2+\sigma^2}\bigg)  .
$$
\bigskip
...
With reference to the hyppocampus model:
...
* Compare the analytical posterior and the numerical posterior obtained via gridding (you create a grid of values of $\mu$, multiply prior and likelihood, and normalize). 
...
# Effect of the scale parameter $\xi$
...
* Compute the posterior distribution of the height model by using a bi-dimensional gridding.

\bigskip

* Assume to have collected two observations: 168 cm. and 178 cm. 

\bigskip

* Plot the posterior joint and the posterior marginals.

\bigskip

* Solution: see code on *icorsi*.
...
# Posterior mean for the hyppocampus model
...
\underbrace{\frac{n\sigma_{\mu}^2}{n\sigma_{\mu}^2+\sigma^2}}_{1-w}, \; \frac{\sigma_{\mu}^2\sigma^2}{n\sigma_{\mu}^2+\sigma^2}\bigg) \\
\\
& \mu' = 6.5 \;\; \sigma_{\mu} = 0.05; \sigma = 0.25
\end{align*}

* For $n$=5, we get $w$=0.833 and  posterior mean 6.53.

\bigskip

* For $n$=35, we get $w$=0.416 and  posterior mean 6.61.

\bigskip

* When we have more data, the posterior mean is closer to the sample mean.
...
plt.subplot(1, 3, 3)
plt.plot(mu, posterior, label='mu = %s\n sigma = %s' % ("{:.1f}".format(post_mean), "{:.1f}".format(post_s)))
plt.legend(fontsize=12)
plt.title('Posterior')
```
...
```{python, echo=TRUE, fig.height=3, fig.align="center", eval=FALSE}
plt.figure(figsize=(10, 3))
mu = np.linspace(-6, 21, 100)
...
```{python, echo=TRUE, fig.align="center", eval=FALSE}
#posterior
y_bar     = np.mean(y)
n         = len(y)
