...
* If we had more than one reasonable prior, we could perform a sensitivity analysis, that is, evaluate how the results change with the priors, we could conclude that for a wide range of priors, the results do not vary!
...
* Let us consider the uniform prior, $y$=8, $n$=18
...
$$ p_{\text{post}} = 0.45 \leq \theta \leq 0.55 = \int_{0.45}^{0.55} p_{\text{post}}(\theta) d\theta $$
```{python, echo=TRUE, fig.align='center'}
from scipy.stats import beta
beta.cdf(0.55, 9, 19) - beta.cdf(0.45, 9, 11)
```
...
* The two terms are practically equivalent.
* However, in principle HPD refers to a *posterior* distribution.
...
```
* The height of the horizontal arrow marks the minimal density exceeded by all $x$ values inside the 95% HDP


# HPD of bimodal distribution
```{r, out.width='45%', fig.align='center', echo=FALSE}
knitr::include_graphics('HPD_bimodal.png')
```

* The HPD is split into two sub-intervals, one for each mode of the distribution. 

* The  characteristics are  before: 
...
# Evaluating the model via predictive test of the posterior

* The _a posteriori_ is used to generate data $\tilde{y}$, that is, predicted data conditioned by the estimated values of $\theta$ and by the already observed data $y$.

$$
p(\tilde{y} \mid y) = \int p(\tilde{y} \mid \theta) p(\theta \mid y) d\theta
$$

* This represents  data tht are expected to be seen in a future experiment.
...
* Try to understand  systematic discrepancies between simulated and observed data, and  revise the model accordingly.

# EXPAND MORE THE predictive test of the posterior??
